...
void remove_duplicates(SinglyLinkedListNode* head) {
    ...
    SinglyLinkedListNode* next_of_next;
    ...
    while (current->next != NULL)
    {
       ...
       {
           //firstly:  skip this duplication
           next_of_next = current->next->next;
           //secondly: free this node
           free(current->next);
           //finally: making the unique non-duplicated node as ur next node
           current->next = next_of_next;
       }
       else
       {
          // don't change anything if it is unique and not a duplicated node
